...
if [ -n "$REDIS_HOST" ]; then
  ...
  sed -i "s/#CACHE_HOST=/CACHE_HOST=$REDIS_HOST/" .env
  sed -i "s/#CACHE_DATABASE=2/CACHE_DATABASE=$REDIS_DATABASE/" .env
  sed -i "s/CACHE_DRIVER=file/CACHE_DRIVER=redis/" .env
fi

if [ -n "$REDIS_PORT" ]; then
  echo "Setting REDIS_PORT"
  sed -i "s/#CACHE_PORT=/CACHE_PORT=$REDIS_PORT/" .env
fi

if [ -n "$REDIS_PASSWORD" ]; then
  echo "Setting REDIS_PASSWORD"
  sed -i "s/#CACHE_PASSWORD=/CACHE_PASSWORD=$REDIS_PASSWORD/" .env
fi
